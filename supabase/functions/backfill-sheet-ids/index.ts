...
Deno.serve(async req=>{
  try{
    ...
    const response=await fetch(Deno.env.get('APPS_SCRIPT_URL')!,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({secret:Deno.env.get('SHEET_SYNC_SECRET'),action:'backfillIds',clients:(clients??[]).map(client=>({row:client.source_row,permanentId:client.permanent_id,sheetName:client.source_sheet_name}))})})
